...
def find_top_k_sentences(query, documents, k):
    ...
    for i in range(0,len(documents)):
      ...
      z.append("Headline:"+query[i] + ". Article:"+ fin )
    return(z)
...
def generate(given_line):
    response = []
    palm.configure(api_key='AIzaSyCF9H4AX3XB1wsG9_F6GQeGMwc1lIN9pRM')
    for i in range(0,3):
...
def get_documents(I,df):
    documents = []
    for i in I:
        text_for_desired_id = df.loc[df['id'] == str(i), 'text'].values[0]
...
iterator = 0
output_json = []
output_file_path = "predict-news.json"

final_json = {"task":"LAMP_4","golds":[]}
with open("train_headlines.json", 'r', encoding='utf-8') as f:
        ...
        for item in json_parser:
            ...
            query = query + " while using writing style of the user's headlines of previous articles written by the user"

           
            output = palm.generate_text(prompt=original_query+". Previous Articles: "+ k)
